...
# User management to maintain separate chatbot instances
class ChatbotManager:
    ...
    def get_chatbot(self, user_id):
        if user_id not in self.chatbot_instances:
            # Create a new instance for each new user
            self.chatbot_instances[user_id] = Chatbot(self.file_paths, self.api_key)
...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["https://myfrontend.com"],  # Specify the frontend URL
#     allow_credentials=True,
#     allow_methods=["*"],  
#     allow_headers=["*"],
# )
